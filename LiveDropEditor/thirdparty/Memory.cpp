...
const char* Memory::ReadText(HANDLE processHandle, int address) {
	if (address == -1)
		return "-1";
	char buffer = !0;
...
const char* Memory::ReadPointerText(HANDLE processHandle, int startAddress, int offsets[], int offsetCount) {
	if (startAddress == -1)
		return "-1";
